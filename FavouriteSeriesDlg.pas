...
type
  TFavouriteSeriesDlg = class(TForm)
    OKBtn: TButton;
    CancelBtn: TButton;
    Label1: TLabel;
    Edit1: TEdit;
  ...
  private
    ...
    FSeriesData: TArray<TJSONObject>; // Store movie data for virtual ListView
    FPosterCache: TArray<Integer>;   // Cache for poster image indices
    procedure ClearSeriesData;
  ...
  public
    ...
    SelectedSeriesYear: string;
    SelectedSeriesOverview: string;
    SelectedSeriesPosterPath: string;
    function GetSelectedSeriesData: Boolean;
  end;

var
  FavoriteSeriesDlg: TFavouriteSeriesDlg;

implementation

{$R *.dfm}

procedure TFavouriteSeriesDlg.FormCreate(Sender: TObject);
begin
...
procedure TFavouriteSeriesDlg.FormDestroy(Sender: TObject);
begin
  ClearSeriesData;
...
procedure TFavouriteSeriesDlg.SetupListView;
begin
...
procedure TFavouriteSeriesDlg.ClearSeriesData;
var
  I: Integer;
begin
  for I := 0 to High(FSeriesData) do
    if Assigned(FSeriesData[I]) then
      FSeriesData[I].Free;
  SetLength(FSeriesData, 0);
  SetLength(FPosterCache, 0);  // Clear poster cache too
end;

procedure TFavouriteSeriesDlg.Edit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
function TFavouriteSeriesDlg.LoadPosterFromURL(const PosterURL: string): Integer;
var
...
procedure TFavouriteSeriesDlg.ListView1Data(Sender: TObject; Item: TListItem);
var
  ...
  ImageIndex: Integer;
begin
  if (Item.Index >= 0) and (Item.Index < Length(FSeriesData)) then
  begin
    SeriesObj := FSeriesData[Item.Index];

    // Get movie data safely
    if not SeriesObj.TryGetValue<string>('name', Title) then
      Title := 'Unknown Title';
    if not SeriesObj.TryGetValue<string>('first_air_date', ReleaseDate) then
      ReleaseDate := '';
    if not SeriesObj.TryGetValue<string>('overview', Overview) then
      Overview := '';
    ...
    begin
      ...
      ImageIndex := -1;
      V := SeriesObj.Values['poster_path'];
...
procedure TFavouriteSeriesDlg.Button1Click(Sender: TObject);
const
...
begin
  ...
  URL := Format('https://api.themoviedb.org/3/search/tv?query=%s&api_key=%s&language=en-US',
    [SearchTitle, API_KEY]);
  ...
  try
    ...
    try
      ...
      try
        ...
        SetLength(FSeriesData, Results.Count);
        for I := 0 to Results.Count - 1 do
        begin
          SeriesObj := Results.Items[I] as TJSONObject;
          // Clone the JSON object to store it
          FSeriesData[I] := SeriesObj.Clone as TJSONObject;
        ...
        try
          ...
          ListView1.Items.Count := Length(FSeriesData);
        finally
...
procedure TFavouriteSeriesDlg.ListView1SelectItem(Sender: TObject; Item: TListItem; Selected: Boolean);
begin
  // Enable/Disable OK button based on selection
  OKBtn.Enabled := Selected ;                  // Not needed and (ListView1.Selected <> nil)
end;

procedure TFavouriteSeriesDlg.OKBtnClick(Sender: TObject);
begin
  if GetSelectedSeriesData then
  begin
    ModalResult := mrOK;
  end
  else
...
function TFavouriteSeriesDlg.GetSelectedSeriesData: Boolean;
var
  SeriesObj: TJSONObject;
  V: TJSONValue;
begin
  ...
  if (ListView1.Selected = nil) or
     (ListView1.Selected.Index >= Length(FSeriesData)) then
    Exit;

  SeriesObj := FSeriesData[ListView1.Selected.Index];

  // Get selected movie data
  if not SeriesObj.TryGetValue<string>('name', SelectedSeriesTitle) then
    SelectedSeriesTitle := 'Unknown Title';

  if not SeriesObj.TryGetValue<string>('first_air_date', SelectedSeriesYear) then
    SelectedSeriesYear := ''
  else
    SelectedSeriesYear := Copy(SelectedSeriesYear, 1, 4); // Just the year

  if not SeriesObj.TryGetValue<string>('overview', SelectedSeriesOverview) then
    SelectedSeriesOverview := '';

  // Get poster path
  SelectedSeriesPosterPath := '';
  V := SeriesObj.Values['poster_path'];
  if (V <> nil) and not (V is TJSONNull) then
    SelectedSeriesPosterPath := V.Value;
